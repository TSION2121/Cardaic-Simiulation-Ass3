...
void cmdLine(int argc, char *argv[], double& T, int& n, int& px, int& py,
             int& plot_freq, int& no_comm, int& num_threads);

// m_total and n_total are the total number of rows and columns respectively
void simulate (double** E,  double** E_prev,double** R,
    ...
    for (j=1; j<=m; j++){
       for (i=1; i<=n; i++) {
          E[j][i] = E_prev[j][i]
                    + alpha*(E_prev[j][i+1]+E_prev[j][i-1]
...
int main (int argc, char** argv)
{
    ...
    int rbeg =0 , rend = 0;
    rbeg = rowID*(rinterval) + 1; // starting from 1
    rend = rbeg + row -1;
    ...
    while (t<T) {
       ...
       simulate(E, E_prev, R, alpha, n, m, kk, dt, a, epsilon, M1, M2,
                b, col, row, px, py, num_threads);
...
// -------------------------------------------------------------------
// Minimal implementations to make the standalone build work in WSL.
// -------------------------------------------------------------------

// Simple command-line parser for this assignment.
void cmdLine(int argc, char *argv[], double& T, int& n, int& px, int& py,
             int& plot_freq, int& no_comm, int& num_threads)
{
    for (int i = 1; i < argc; ++i) {
        if (strcmp(argv[i], "-n") == 0 && i+1 < argc) {
            ...
            py = atoi(argv[++i]);
        } else if (strcmp(argv[i], "-plotfreq") == 0 && i+1 < argc) {
            plot_freq = atoi(argv[++i]);
        } else if (strcmp(argv[i], "-nocomm") == 0 && i+1 < argc) {
            no_comm = atoi(argv[++i]);
...
// Dummy plotting function: do nothing when called.
extern "C" void splot(double **E, double T, int niter, int m, int n)
{
    // Intentionally empty; real version would call gnuplot.
}
